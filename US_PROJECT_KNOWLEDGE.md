...
- 2026-09-11 ("안 도는 것 같다" 진단 → v09, patch_note/v09_20260911.md): Actions 는 매일 success 인데
  us_latest.csv 가 09-08 데이터로 정지, score_daily 20260909·10 공백. 실측 원인 3중 — ① 09-02 부터 실행 당일
  시세가 44~80%만 수집(러너 #52: `배치 실패 0` 인데 20260910 5,226/6,558, 정렬 뒤쪽 S~Z 두 분위 결측 86~89%,
  직후 야후 429/401 크럼 거부 — 예외 없이 조용히 빠짐. 다음날 7일 창이 채워 DB 는 완결) ② v08 게이트가 최신일만
  보고 조기 반환 → 채워진 전날을 영영 계산 안 함 ③ 휴장일 20260907 잔행(CREG 1행)이 분모가 돼 20260908 이
  유니버스 1,466 으로 통과·박제(잔행 열이 계산 창도 하루 밀었음). **v09**: 잔행 날짜 제외(page_data·notify 동일) ·
  게이트 통과 못 한 날 자동 보충(score_daily 공백일만, 부분 적재된 날은 REPAIR 권고만) · 수집기 2차 수집(결측
  심볼만 90s 뒤 `start=` 명시 소배치). 오프라인 검증: 20260904(잔행 없음) 구·신 score_daily 완전일치, 20260909 는
  구코드가 잔행 열로 창이 하루 밀려 있었음(신코드가 정정, top50 겹침 41/50). 러너 #53 수동 실행
  `repair_dates=20260902,20260903` 완료(1,816→3,371 · 2,378→3,365, 구코드로 충분 — 창에 잔행 없음). 같은 실행이 장 시작 전
  이라 20260910 을 3,350 으로 정상 적재(단 구코드 = 잔행 열 포함). **남은 사용자 조치**: v09 커밋·푸시 → Run workflow
  `repair_dates=20260908,20260910`(잔행 열 제외 재계산) 1회. 20260909 는 자동 보충.
  별건 관찰: us_rotate_collector 야후 401(Invalid Crumb) 연발·섹터 캐시 +0, 절벽 큐 회당 200 중 197 "잔존 — 봉인 안 함"
  반복(6회 후 자기 종료).

- 2026-09-11 (새 재료 2건 스캔, 같은 날 저녁): ① **내부자 클러스터 매수 진짜 플래그 재실행**
  (research/RESEARCH_us_orth_rerun_20260911.md, 09-06 스크립트 무수정·29분기 플래그 전부 복구): 클러스터&순매수비>0.5
  가 +1.02%p Bonf 통과(프록시) → **+0.56%p 95%[+0.01,+1.11] Bonf6 미달**, 2025 +0.17·2026 −0.01 무신호. 프록시가
  '오래 재직한 반복 매수자'만 잡아 부풀렸던 것으로 추정(사후 가설, 이번 라운드 불채택). top50 증분은 7회째 무신호/음.
  **→ PREREGISTER 후보에서 제외("채택 안 함").** ② **FINRA 일별 공매도 비율 첫 스캔**(research/RESEARCH_us_shortvol_scan_20260911.md,
  2023-05~2025-07 적재분, n=92/54): svr5·svr20·Δsvr IC −0.009/−0.009/−0.002 전부 CI 0 포함, 십분위 단조성 없음, top50
  짝비교 3종 CI 0 포함 → **무신호**. 백필이 2026 까지 차면 1회 재실행 허용. 옵션(8일)은 아직 표본 없음.
  현재 남은 후보는 indmom(09-06 아키텍처 스캔)뿐.
...
- **야후 당일 시세 조용한 부분 수집 (2026-09-02~ 실측, v09 대응)**: 증분 fetch 가 예외 없이 끝나는데 실행 후반
  심볼(정렬 S~Z)의 당일 행만 빠진다(러너 #52: 5,226/6,558, 9·10분위 결측 86~89%, TSLA·TSM 포함). 직후 다른
  수집기에서 `Crumb fetch rate-limited (HTTP 429)`·`401 Invalid Crumb`. 스로틀(요청 수 누적) vs 캐시(전날 응답)
