...
- **네트워크(Codex 로컬 실행 기준)**: 이 환경은 사용자 노트북에서 돌아가므로
...
- **git(Codex 기준)**: `git status/diff/log/show` 같은 읽기 명령은 허용.
...
- 점수/동작을 바꾸는 변경은 오프라인 0-diff 검증 후에만 제안, 사용자 승인 후 적용.
- 동작이 바뀌는 모든 변경(수집기·notify·워크플로·페이지·스코어)은
  patch_note/vNN_YYYYMMDD.md 패치노트를 함께 작성한다 — 무엇을/왜(실측 근거)/
  바뀐 파일/검증/남은 한계. 정본 규칙은 patch_note/README.md. 마지막 보고의
  커밋 목록에 패치노트를 포함한다.
- 연동 폴더에서 git 명령을 실행하지 않는다(브리지가 파일 삭제 불가라 index.lock이
  남아 사용자 커밋을 막은 실측 사고, 2026-08-19). 파일 읽기/쓰기만 하고,
  커밋·푸시는 사용자가 GitHub Desktop 으로 한다.

[보고 규칙]
- IC·수익은 항상 표본수(n)·CI와 함께. in-sample/생존편향 미보정 여부를 명시.
- 직접 관찰과 추정을 구분해 표기한다. 간결한 한국어.
- [동기화 의무] 폴더가 세션에 연동돼 있으므로 별도 '지식 업로드' 단계는 없다.
  세션 중 수정/신규 파일(코드·지식 문서·패치노트 포함)이 있으면 마지막 보고에
  "🔄 git 커밋 필요: [파일 목록]"을 출력한다. 해당 없으면 "커밋 필요 없음"을 명시한다.
