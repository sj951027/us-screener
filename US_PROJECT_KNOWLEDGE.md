...
- 2026-09-12 (v09 첫 스케줄 실행 결과 → v10 계측, patch_note/v10_20260912.md): 러너 #55 가 09-11 시세를
  **1,055/6,551(16%)** 만 받았다. v09 안전망은 설계대로 동작(게이트 차단·경고 전송·오염 0). 그러나 **v09 2차 수집은
  +0행으로 무효 판명** → 제거. 같은 날 대조 실험으로 좁힌 것: 데이터는 야후에 있고(로컬 20/20), 라이브러리 버전
  문제도 아니며(러너와 동일한 yfinance 1.7.0 + pandas 3.0.5 로 로컬 29/30 정상), 결측이 알파벳 10분위에 균일해
  배치 순서·누적 요청량 탓도 아니다. 대신 **하이픈 심볼 93% vs 일반 심볼 11%** 라는 구조적 차이와 러너 지역 차이
  (#53·#54 westcentralus 정상 / #55 eastus2 16%)가 남았다. **원인 미특정** — 수집기가 심볼별 실패를 삼켜 기록이
  없었던 것이 병목이라, 추측 기반 수정(옵션 축소·cron 추가) 대신 **v10 계측**을 넣었다(출구 IP·사유별 집계·결측 3분류·
  의존성 고정). 다음 실행 로그 1회로 좁힌다. **러너 #56(09-12 02:26 UTC, v10) 검증 완료**: 계측 정상 출력 + 09-11 을 6,543/6,551(99.9%)로 복구 → score_daily 20260911 3,352행 · 페이지 기준일 20260911(커밋 62b9aca). 정상 실행 기준선 = 결측 3 · 경고 11건. 지문 누적: #53·#54 westcentralus 정상 / #55 eastus2 16% / #56 westus3 IP 20.171.126.211 정상 — 4건 중 실패는 eastus2 뿐이나 표본이 작아 관찰 단계.

- 2026-09-16 ("시장을 크게 이기는 종목" 스캔, research/RESEARCH_us_bigwin_scan_20260916.md · 09-16 정본 tar ·
  앵커 109개 2024-06→2026-08 · in-sample): ① **큰 승자(+50%/60d)를 2~4배 예고하는 사전 특징(고변동·52주고점
  이탈·저수익성·저가·소형·상장 1년 미만)은 전부 복권형** — 같은 그룹의 −40% 급락도 2.7~4배, 60일 평균 초과 0 이하.
  승자 확률↑·급락↓·평균 초과 양(+)을 다 갖는 건 추세 계열만(base top50 +4.2%p/60d [+0.1,+8.0]*, top10 +9.2*).
  ② **매출성장(rev_g, 분기 YoY)이 직교 재료 중 처음으로 top50 증분 점추정이 전 변형 양(+)**: 4항 Δ +0.64%p
  [+0.04,+1.24]* 이나 커버리지 대조군 대비 +0.55 [−0.01,+1.14] 로 95% 경계 미달·Bonf12 미달·연도별
  +1.39/+0.68/−0.09 로 2026 소멸 → 채택 아님, score_daily 관측 컬럼 후보(패치노트·승인 필요). sp·opm 은 "전
  유니버스 유의 ≠ top50 증분" 8회째, asset_g 는 사전부호 역전 탈락. ③ 레짐 타이밍(200MA·시장폭·VIX 현금화)은
  전부 해로움(체인 +67%→+14~37%), SPX≤200MA 앵커(n=11, 2025-04 반등)에서 초과 최대 — 약세장 표본 0. ④ 라이브
  **us_mus_v0 top50 07-13→09-14 (44일) −10.75% vs 가드EW −0.58% vs SPX +1.39% = −10.2%p**(MNST 절벽 −1.27%p
  포함, 반도체·광통신 군 낙폭), us_rvdtc_a −11.4%p. ⑤ 일별 공매도 svr 2026 확장 재실행 무신호 → 종결.
  데이터 관찰: **20260915 시세 16% 부분 수집**(게이트 정상 차단) · **MNST 절벽 09-16 정본에서도 미수정**(v08 큐
  소화 확인 필요) · 내부자 구조화셋 2026-03-31 까지.
- 2026-09-17 (부분 수집 원인 특정 → v11 cron 이동, patch_note/v11_20260917.md): Actions 로그 32회 대조 결과
  **당일 시세 16% 부분 수집은 실행 시각 문제**. cron 22:00 UTC 가 9월부터 GitHub 지연(~2시간)으로 23:50~00:25 UTC
  시작 → 시세 루프가 00:00 UTC(=20:00 ET) 이전에 끝나면 정상, 걸치면 31~80%(뒤 알파벳 결측), 이후면 16%(4회 연속
  동일). 8월 22:30 UTC 실행·수동 02:26/10:49/12:45 UTC 는 전부 정상. v10 계측상 429/401 0 · 응답 누락 0 → 차단
  아님. eastus2 '지역 차이'는 시각 교란이었다. 대응: cron `17 3 * * 2-6` + ET 세션 날짜 판정(ET 06시 이전 = 전날).
...
- **수집 실패를 볼 수 있게 하는 법 (v10, 2026-09-12)**: 러너 로그에서 순서대로 본다 — ① `[환경] 출구IP … · yfinance …`
  (러너별 대조·설치본 확인) ② `증분 완료: 신규 N행` ③ `[증분 결측] 빈 프레임 / 응답에 심볼 없음 / 예외`
  ④ `[증분 사유]` 상위 메시지(HTTP 본문 포함) ⑤ 수확량 점검의 하이픈/일반 비율. Azure 지역은 로그 헤더
  `Azure Region:` 에 원래 찍힌다. 진단 요령(정본 tar): 날짜별 심볼 수 + 결측 심볼의 알파벳 10분위 분포
  (후반 편중이면 배치 순서/스로틀) + 하이픈 포함 여부별 수집률(구조적 차이). 로컬 대조는 러너와 같은 버전으로
  venv 를 만들어 같은 코드 경로를 태운다 — 이것으로 '데이터 없음'과 '러너에서만 실패'를 가른다.
